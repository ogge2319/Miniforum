...
export const handlers = [
  ...
  http.post("/auth/login", async ({ request }) => {
    const body = await request.json();          // <-- läs body
  ...
  http.put("/me/consents", async ({ request }) => { // <-- ledande slash
    const body = await request.json();
    me.consents = body;
    return HttpResponse.json(me.consents);
  }),

  http.post("/me/export", () => HttpResponse.json({ status: "export_requested" })), // <-- ledande slash
